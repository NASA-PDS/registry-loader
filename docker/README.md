...
```
    docker image build --target full --tag nasapds/registry-loader --file docker/Dockerfile --build-arg harvest_package_path=harvest/target/harvest-5.0.0-SNAPSHOT-bin.tar.gz --build-arg reg_manager_package_path=manager/target/registry-manager-6.0.0-SNAPSHOT-bin.tar.gz .
```
...
```
    docker image build --target lite --tag nasapds/registry-loader-lite --file docker/Dockerfile --target lite --build-arg harvest_package_path=harvest/target/harvest-5.0.0-SNAPSHOT-bin.tar.gz --build-arg reg_manager_package_path=manager/target/registry-manager-6.0.0-SNAPSHOT-bin.tar.gz .
```
